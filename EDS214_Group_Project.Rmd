...
```{r}
#Potassium and Nitrate (all available years)

#add mean K and N values to joined data (including data for all available years)
Joined <- Joined %>% 
  group_by(monthyear, Sample_ID) %>% 
  mutate(meanK = mean(K, na.rm=TRUE), meanN = mean(`NO3-N`, na.rm=TRUE)) %>% 
  select(Sample_ID, monthyear, meanK, meanN) %>% 
  distinct(monthyear, meanK, meanN)
```
